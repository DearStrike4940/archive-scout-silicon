...
- `scripts/build_macos_apple_silicon.sh`: Apple Silicon `.app` and DMG.
...
PyInstaller uses `--onedir` for faster repeated launches instead of unpacking a one-file runtime every time. The workflow builds on `macos-15` arm64, verifies the runner architecture, ad-hoc signs the app bundle, and creates a compressed DMG with an Applications shortcut.
